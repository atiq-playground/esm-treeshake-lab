...
Push, then open the PR yourself as a **draft** — always, even on a `BLOCK` verdict; the review is advisory, not a gate that withholds the PR — with `Closes #<n>` in the body, following the Pull Request Workflow in [git-workflow.md](../../rules/common/git-workflow.md): full commit history, diff against the base branch, a comprehensive summary, and a test plan.
